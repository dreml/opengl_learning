...
main :: proc() {
	...
	for !glfw.WindowShouldClose(window) {
		...
		light.position = glm.vec3{light.position.x, f32(math.sin(glfw.GetTime())), f32(math.cos(glfw.GetTime())) * 2 - 2}

		gl.UseProgram(lightProgram)
		gl.BindVertexArray(lightVAO)
		model := glm.mat4(1)
		model *= glm.mat4Translate(light.position)
		...
		gl.Uniform3fv(gl.GetUniformLocation(mainProgram, "light.position"), 1, &light.position[0])
		gl.Uniform3fv(gl.GetUniformLocation(mainProgram, "light.ambient"), 1, &light.ambient[0])
		gl.Uniform3fv(gl.GetUniformLocation(mainProgram, "light.diffuse"), 1, &light.diffuse[0])
		gl.Uniform3fv(gl.GetUniformLocation(mainProgram, "light.specular"), 1, &light.specular[0])
		gl.Uniform3fv(gl.GetUniformLocation(mainProgram, "viewPos"), 1, &camera.pos[0])
		...
		gl.Uniform3fv(gl.GetUniformLocation(mainProgram, "material.ambient"), 1, &material.ambient[0])
		gl.Uniform3fv(gl.GetUniformLocation(mainProgram, "material.diffuse"), 1, &material.diffuse[0])
		gl.Uniform3fv(gl.GetUniformLocation(mainProgram, "material.specular"), 1, &material.specular[0])
		gl.Uniform1f(gl.GetUniformLocation(mainProgram, "material.shininess"), f32(material.shininess))
